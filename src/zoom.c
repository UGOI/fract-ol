...
t_vector	zoom_point_to_coordinte(t_fractal zoomed_fractal, mlx_image_t *img, t_vector zoom_point_screen)
{
	t_vector	zoom_point_coordinate;

	zoom_point_coordinate.x = (zoomed_fractal.top_left.x) + (zoom_point_screen.x)
		/ (img->width) * (zoomed_fractal.top_right.x
			- zoomed_fractal.top_left.x);
	zoom_point_coordinate.y = (zoomed_fractal.top_left.y) - (zoom_point_screen.y)
